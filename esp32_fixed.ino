...
void loop() {
  if (deviceConnected) {
    ...
    String gpsData = String(currentLat, 6) + "," +
                     String(currentLng, 6) + "," +
                     String(currentAlt, 2);

    // Send via BLE - Use false for notify() to send only notification, not indication
    pCharacteristic->setValue(gpsData.c_str());
    pCharacteristic->notify(false);  // false = send only notification, not indication

    // Log to Serial Monitor
    Serial.print("Sent GPS (");
    Serial.print(gpsData.length());
    Serial.print(" bytes): ");
